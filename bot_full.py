...
def welcome(message):
    ...
    item_3 = types.KeyboardButton("Zufallsatz")
...
def lalala(message):
    if message.chat.type == 'private':
        ...
        # elif message.text == 'Zufallsatz':
        #     bot.send_message(message.chat.id, "Das ist ein Zufallssatz!")
        else:
...
def callback_inline(call):
    try:
        if call.message:
            ...
            bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id, text="😊 Wie geht es Dir?", reply_markup=None)
            # show alert
